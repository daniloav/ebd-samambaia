...
| **V31** | `V31__leitura_biblica_diaria.sql` | Leituras bíblicas diárias da lição: tabela `aula_texto_biblico` (`aula_id` FK `ON DELETE CASCADE`, `dia_semana`, `referencia`, `texto_cache`, `texto_cache_em`, `enviado_em`), unique `(aula_id, dia_semana)` e check no dia da semana. Cadastro opcional por aula; `enviado_em` deduplica o envio diário das 12h e `texto_cache` guarda o texto buscado na internet. |
