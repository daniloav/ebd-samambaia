...
| **V31** | `V31__leitura_biblica_diaria.sql` | Leituras bíblicas diárias da lição: tabela `aula_texto_biblico` (`aula_id` FK `ON DELETE CASCADE`, `dia_semana`, `referencia`, `texto_cache`, `texto_cache_em`, `enviado_em`), unique `(aula_id, dia_semana)` e check no dia da semana. Cadastro opcional por aula; `enviado_em` deduplica o envio diário das 8h e `texto_cache` guarda o texto buscado na internet. |
...
| **V32** | `V32__aula_reposicao_de.sql` | Vínculo da **reposição** com a aula adiada que a originou: coluna `reposicao_de_id` (FK self em `aula`, `ON DELETE SET NULL`) + índice. É o que permite **retirar o adiamento** (excluir a reposição e puxar a agenda -7 dias). Faz **backfill** best-effort das aulas já adiadas: casa a aula da mesma turma 7 dias depois com o mesmo tema; quem não casar fica sem vínculo (o desfazer então só tira a marca, sem excluir nada por adivinhação). |
| **V33** | `V33__requisicao_juntada.sql` | **Juntar requisições** da tesouraria: colunas `juntada_na_id` (FK self em `requisicao_tesouraria`, `ON DELETE SET NULL`) + `juntada_em` + índice. A absorvida fica com status `JUNTADA` apontando para a principal e **mantém o próprio valor** — é o que permite desfazer a junção subtraindo exatamente o que entrou. A coluna `status` é `VARCHAR(12)` sem CHECK, então `'JUNTADA'` cabe sem alteração. Estende a view `vw_requisicoes_integracao` (append-only) com `juntada_na` e `qtd_juntadas`. |
| **V34** | `V34__prova_recuperacao.sql` | **Prova de recuperação**: `prova.tipo` passa a `VARCHAR(12)` (cabe `'RECUPERACAO'`), nova `prova.aula_id` (FK `aula`, `ON DELETE CASCADE`) + índice e unique parcial `uq_prova_recuperacao_aula` (**uma recuperação por aula**). `submissao` ganha `tentativa` (default 1 — as submissões existentes viram a 1ª tentativa) e a unique `(prova_id, aluno_id)` vira `uq_submissao_tentativa (prova_id, aluno_id, tentativa)`, permitindo as 3 tentativas. |
