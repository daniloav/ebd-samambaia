...
- 📝 **Falta justificada (30%) + status automático (2026-07-28)** — migration **V24** (`justificada`/`justificativa_motivo`/`justificada_em` em `presenca`). (1) O aluno **justifica a própria falta** em `/minha-frequencia` (autosserviço, vale na hora): uma falta justificada passa a valer **0,3** (30% de uma presença) no ranking *menos faltou* e na *classificação geral* (`DesafiosService`); endpoints `POST/DELETE /api/me/frequencia/{aulaId}/justificar`. (2) Ao salvar a chamada, aluno com **>4 faltas seguidas sem justificativa** (5ª) é **inativado** automaticamente (presença ou falta justificada zera a sequência) — `ChamadaService`, com alerta no toast + Auditoria. (3) Ao cadastrar visitante, quem foi às **3 aulas seguidas** mais recentes da turma (identidade = nome + telefone/e-mail) **vira aluno** com login automático — `VisitanteService` + `AcessoAlunoService`. Validado: `FaltaJustificadaEInativacaoTest` (5 casos), **50** testes verdes, `ng build`. Os 2 eventos automáticos também enviam e-mail (aluno inativado; visitante promovido) via `NotificacaoService`, respeitando o toggle `ebd.notificacoes.enabled`.
